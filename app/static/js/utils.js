...
export class UtilsItemColorTransition {
  ...
  updateStyles(isEditable) {
    if (isEditable) {
      ...
      this.form.removeClass("opacity-0 group-hover:opacity-100");
      ...
      this.form.addClass("opacity-0 group-hover:opacity-100");
...
export function handleUtilsItemUpdate(element, apiRoute) {
  element.submit(function(event) {
    ...
    fetch(apiRoute, {
      ...
      body: JSON.stringify({
        "todolist-item-edit-prev": $(this).closest(".utils-item").find("input.utils-item-delete-input").val(),
        "todolist-item-edit": $(this).find("input.utils-item-edit-input").val(),
      }),
    })
      .then(response => response.json())
      .then(todolistEditValues => {
        $(this).closest(".utils-item").find("input.utils-item-delete-input").val(todolistEditValues["todolist-item-edit"]);   // otherwise below function will fail with consecutive updates
      })
      .catch(error => {
        console.error(error);
      })
...
export function handleUtilsItemDeletion(element, apiRoute) {
  element.submit(function(event) {
    event.preventDefault();
    fetch(apiRoute, {
      method: "POST",
