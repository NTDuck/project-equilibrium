...
def add_todolist_item():
    if "todolist-item-add" in request.form:
        todolistItemEventHandler.handle_db_insert("todolist-item-add")
...
def edit_todolist_item():
    if "todolist-item-edit" in request.form:
        todolistItemEventHandler.handle_db_update("todolist-item-edit-prev", "todolist-item-edit")
    return redirect(url_for("main.index"))
...
def delete_todolist_item():
    if "todolist-item-delete" in request.form:
        todolistItemEventHandler.handle_db_delete("todolist-item-delete")
    return redirect(url_for("main.index"))